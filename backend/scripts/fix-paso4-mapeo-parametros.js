...
async function fixPaso4() {
  try {
    ...
    console.log('📝 ANTES:');
    console.log('   mapeoParametros:', JSON.stringify(workflow.steps[paso4Index].mapeoParametros, null, 2));
    console.log('');

    // Actualizar el mapeoParametros del paso 4
    await db.collection('api_configurations').updateOne(
      { _id: api._id },
      {
        $set: {
          [`workflows.${workflowIndex}.steps.${paso4Index}.mapeoParametros`]: {
            search: {
              origen: 'variable',
    ...
    console.log('');
    console.log('📝 DESPUÉS:');
    console.log('   mapeoParametros: {');
    console.log('     search: {');
    console.log('       origen: "variable",');
    console.log('       nombreVariable: "titulo"');
    console.log('     }');
    console.log('   }');
