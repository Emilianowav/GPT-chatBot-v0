...
export class FlowExecutor {
  ...
  private async executeGPTNode(node: any, input: any): Promise<NodeExecutionResult> {
    ...
    // Guardar variables globales si están configuradas (legacy + auto-extracción para GPT conversacional)
    if (config.globalVariablesOutput && Array.isArray(config.globalVariablesOutput) && config.globalVariablesOutput.length > 0) {
      console.log(`\n🔍 [DEBUG] Procesando globalVariablesOutput (${config.globalVariablesOutput.length} variables)`);
      ...
      // Si hay variables faltantes, es un nodo GPT conversacional, y tiene respuesta_gpt, intentar extracción
      if (variablesFaltantes.length > 0 && config.tipo === 'conversacional' && output.respuesta_gpt) {
        console.log(`   ⚠️  Variables faltantes: ${variablesFaltantes.join(', ')}`);
        console.log(`   🤖 Intentando auto-extracción desde historial completo...`);
